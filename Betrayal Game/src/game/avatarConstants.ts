...
export function pickAvailableColor(takenColors: string[]): string {
  const available = PLAYER_COLORS.filter((c) => !takenColors.includes(c.id));
  if (available.length === 0) {
    return PLAYER_COLORS[Math.floor(Math.random() * PLAYER_COLORS.length)]!.id;
  }
  return available[Math.floor(Math.random() * available.length)]!.id;
}

export function pickRandomAvatar(): string {
  return PLAYER_AVATARS[Math.floor(Math.random() * PLAYER_AVATARS.length)]!.id;
}
